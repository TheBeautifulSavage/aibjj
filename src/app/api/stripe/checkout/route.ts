...
export async function POST(req: NextRequest) {
  try {
    ...
    const { priceId, planType } = body as {
      priceId?: string;
      planType?: "PRO" | "ANNUAL" | "LIFETIME" | "CREATOR_PRO" | "CREATOR_ELITE";
    };
    ...
    if (planType === "LIFETIME") {
      ...
      const lifetimeSession = await stripe.checkout.sessions.create({
        ...
        line_items: [
          {
            price_data: {
              currency: "usd",
              product_data: {
                name: "AIBJJ Lifetime Membership",
                description: "Founding member — lifetime access to all Pro features. No subscription ever.",
              },
        ...
        success_url: `${process.env.NEXT_PUBLIC_APP_URL}/dashboard?session_id={CHECKOUT_SESSION_ID}&plan=lifetime`,
      ...
      return NextResponse.json({ url: lifetimeSession.url });
    }
